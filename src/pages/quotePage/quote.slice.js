...
const quotePageSlice = createSlice({
  ...
  reducers: {
    ...
    premiumFilterQuotes: (state, action) => {
      console.log(Number(action.payload.split("-")[0]), "++++++");
      state.filterQuotes = state.filterQuotes.map((item) => {
        if(item !== []){
            return item.filter((quote) => {
                if(quote.total_premium > Number(action.payload.split("-")[0]) &&  quote.total_premium < Number(action.payload.split("-")[1])) return quote;
